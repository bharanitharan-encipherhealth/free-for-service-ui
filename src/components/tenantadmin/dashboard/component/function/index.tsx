...
export const getFormattedChartData = (rawSeries: any[], chartType: string) => {
...
export const formatKValue = (val: any) => {
...
export const toFixedNum = (value: any, precision = 2) => {
...
export const filterWidgetsByRole = (widgets: Widget[], selectedRole: string) => {
  if (!Array.isArray(widgets) || !selectedRole) return [];
  const format = (str: string) => str.toLowerCase().replace(/[\s_]/g, "");
  return widgets.filter((widget) => {
    if (!widget.rolesAccessList) return true;
    return widget.rolesAccessList.some(
      (role) => format(role) === format(selectedRole),
    );
  });
};
